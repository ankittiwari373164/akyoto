...
import { Package, ShoppingCart, Users, TrendingUp, Plus, Edit, Trash2, X, Image as ImageIcon, LogOut } from 'lucide-react'
...
export default function AdminPage() {
  ...
  const [productForm, setProductForm] = useState({
    ...
    description: '',
    price: '',
    category: 'Security Alarm System',
    stock: '',
    sku: '',
    image_url: '',
    weight: '1 Unit',
  ...
  // Access to this page is already gated by middleware.ts, which checks the
  // env-based admin session cookie before the page ever renders — there's
  // no Supabase auth/is_admin check here at all.
  ...
  const fetchData = async () => {
    try {
      ...
      const [productsJson, ordersJson, quotesJson] = await Promise.all([
        productsRes.json(), ordersRes.json(), quotesRes.json(),
      ])
  ...
  const resetProductForm = () => {
    setProductForm({ name: '', slug: '', description: '', price: '', category: 'Security Alarm System', stock: '', sku: '', image_url: '', weight: '1 Unit', min_order_qty: '1', specifications: {} })
  }
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold" style={{ fontFamily: 'Manrope, sans-serif' }}>Admin Dashboard</h1>
            <p className="text-slate-500 text-sm mt-1">Signed in with the admin password from <span className="font-semibold text-primary-600">.env.local</span></p>
          </div>
          <button
            ...
            {activeTab === 'products' && (
              <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <select value={productForm.category} onChange={(e) => setProductForm({ ...productForm, category: e.target.value })} className="input-field">
                          <option>Security Alarm System</option>
                          <option>Gas Alarm</option>
                          <option>Carbon Monoxide Alarm</option>
                          <option>Smoke Alarm</option>
                          <option>Composite Alarm</option>
                          <option>Audible and Visual Alarm</option>
                          <option>Wireless Intelligent Doorbell</option>
                          <option>Door Magnet Sensor</option>
                          <option>Intelligent Single Product</option>
                        </select>
                      </div>
                ...
                <div className="overflow-x-auto">
                  ...
                  {!loading && products.length === 0 && <p className="text-center text-gray-500 py-8">No products yet. Add your first one above.</p>}
                </div>
              </div>
            )}
